...
for (i in 1:nrow(CR_SDG_indicators)){
  ...
  for (c in MENARO_metadata$iso3){
    #c <- 'TUN' #to test  
    data.i.c <- data.i |>
      filter(iso3 == c) |>   #data for indicator|country
      arrange(time.period)  #ordering by year to properly assign value in previous measurement
    ...
    if(points.i.c > 1){ 
      ...
      if(identical(base.year.pos, integer(0))){
        ...
        obs.value.base.year <- denominator * exp(wgm(data.i.c, year.to.predict = base.year))
      } else {
      ...
      if(identical(current.year.pos, integer(0))){
        #Estimate base year value when not found
        obs.value.current.year <- denominator * exp(wgm(data.i.c, year.to.predict = current.year))
      } else {
        #Use base year value if found
      ...
      ##### Target year value----
      obs.value.target.year <- denominator * exp(wgm(data.i.c, year.to.predict = target.year))
      ...
      AP.i.c <- CS(I_cv = obs.value.target.year, 
                   ...
                   TV = TV.i, 
                   direction = direction.i)
      AP.score.i.c = ifelse(AP.i.c > 9, "On-track",
                            ifelse(AP.i.c <= 0, 'Regression', 'Slow progress'))
      
    } else if(points.i.c== 1){
      #### 1 data point score ----
        reached.target <- ifelse(direction.i == 'increasing',
                                 data.i.c$obs.value >= TV,
                                 data.i.c$obs.value <= TV)
        ...
        AP.score.i.c <- ifelse(reached.target, 'On-track', 'Insufficient data')
